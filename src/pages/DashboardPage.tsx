...
export function DashboardPage() {
  ...
  const totalWatchHours = stats.hoursWatched;
    ...
    <StudentLayout>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          ...
          <StatCard
            label="চলমান ভিডিও"
            value={stats.inProgressVideos.length.toString()}
          ...
          <section className="mb-10">
            <div className="space-y-4">
              {announcements.map((announcement) => {
                ...
                return (
                  <div key={announcement.id} className={`flex items-start gap-4 p-4 rounded-xl border ${bgClass} shadow-sm`}>
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${iconClass}`}>
                      {icon}
                    </div>
                    <div>
                      <h3 className={`font-medium ${textClass} mb-1 bangla`}>{announcement.title}</h3>
